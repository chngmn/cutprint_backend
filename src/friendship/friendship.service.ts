...
export class FriendshipService {
  ...
  async acceptFriendRequest(
    ...
    // 친구가 된 두 사람의 닉네임 조회
    const receiver = await this.userRepository.findOne({
      where: { id: friendship.receiver_id },
    });
    const requester = await this.userRepository.findOne({
  ...
  async getReceivedFriendRequests(
    userId: number,
  ): Promise<{ id: number; user: User; requested_at: Date }[]> {
    const friendships = await this.friendshipRepository.find({
  ...
  async searchUsers(query: string, currentUserId: number): Promise<any[]> {
    const users = await this.userRepository
      ...
      .where('user.nickname ILIKE :query OR user.email ILIKE :query', {
        query: `%${query}%`,
      })
      ...
      where: [
        {
          requester_id: userId,
          status: 'accepted',
          requester_close_friend: true,
        },
        {
          receiver_id: userId,
          status: 'accepted',
          receiver_close_friend: true,
        },
      ],
      relations: ['requester', 'receiver'],
    });
